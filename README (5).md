...
The `show_gemmimol.py` file contains **two separate docstrings**, and it is
important to know the difference between them:

- **Module docstring** — attached to the file itself. It contains the full
  reference of **keyboard shortcuts** (the same shortcuts shown in the viewer's
  on-screen "H = toggle help" panel), plus notes on how the viewer loads models
  and density maps.
...
```python
import show_gemmimol as gm_module          # the module (full reference)
from show_gemmimol import show_gemmimol     # the function (usage examples)

print("=== MODULE DOC (keyboard shortcuts) ===")
print(gm_module.__doc__)

print("\n=== FUNCTION DOC (usage examples) ===")
print(show_gemmimol.__doc__)
```
...
| Line | Purpose |
|---|---|
| `import show_gemmimol as gm_module` | Imports the **module** under the alias `gm_module`, so its docstring stays reachable. |
